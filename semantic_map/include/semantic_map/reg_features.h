...
#include "/home/younes/Bureau/Pointer2Labs/PFE/catkin_semantic/src/ekz-public-lib/include/mygeometry/KeyPoint.h"
using namespace std;
using namespace cv;
...
public:
    ...
    std::string saveOrbFeatures(SemanticRoom<PointType>& aRoom, std::string roomFolder)
    {
		...
		Ptr<cv::Feature2D> orb = ORB::create(600);
	

		Ptr<FeatureDetector> detector = ORB::create();
    	detector->detect(rgb_image, keypoints);
    	Ptr<DescriptorExtractor> extractor = ORB::create();
    	extractor->compute(rgb_image, keypoints, descriptors );
